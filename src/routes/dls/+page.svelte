<script lang="ts">
	...
	function findPath() {
		path = dls(graph, start.toUpperCase(), goal.toUpperCase(), depthLimit);
	}
...
<div class="space-y-8">
	<div class="bg-white p-6 rounded-lg shadow">
		<h2 class="text-2xl font-bold mb-4">Depth-Limited Search</h2>
		<p class="mb-4">
			DLS is a variant of depth-first search where the search is limited to a maximum depth to prevent
			infinite exploration in deep or cyclic graphs.
		</p>
		...
		<div class="grid grid-cols-3 gap-4 mb-4">
			...
			<div>
				<label class="block mb-2">Depth Limit:</label>
				<input
					type="number"
		...
		{:else if path === null}
			<div class="mt-4 text-red-600">
				<h3 class="font-bold">No path found within depth limit</h3>
			</div>
		{/if}
	</div>
